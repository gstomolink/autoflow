...
export default function BulkImportShopModal({ onClose }: { onClose: ()=>void }) {
  const [csvData, setCsvData] = useState<string[][]>([]);
  const [error, setError] = useState("");
  const [fileName, setFileName] = useState("");

  const headers = ["shopId", "name", "address"];

  const downloadSample = () => {
    const csv = headers.join(",") + "\n" + "shop-002,Colombo City Shop,789 Orchard Rd";
  ...
  const handleUpload = () => {
    ...
    console.log("Uploading shops:", csvData);
    alert("Shops imported successfully (mocked)");
    onClose();
  };
